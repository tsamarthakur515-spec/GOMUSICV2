...
/*
#cgo linux LDFLAGS: -L${SRCDIR}/.. -L. -lntgcalls -lm -lz -lresolv -ldl
#cgo darwin LDFLAGS: -L${SRCDIR}/.. -L. -lntgcalls
#include "ntgcalls.h"
#include <stdlib.h>
...
func NTgCalls() *Client {
	instance := &Client{ptr: uintptr(C.ntg_init())}
...
func handleLogs(logMessage C.ntg_log_message_struct) {
	message := fmt.Sprintf("(%v:%v) %v", C.GoString(logMessage.file), uint32(logMessage.line), C.GoString(logMessage.message))
	lg := loggerNTGCalls
...
func handleStreamEnd(_ C.uintptr_t, chatID C.int64_t, streamType C.ntg_stream_type_enum, streamDevice C.ntg_stream_device_enum, ptr unsafe.Pointer) {
	...
	for _, x0 := range self.streamEndCallbacks {
		go x0(int64(chatID), goStreamType, parseStreamDevice(streamDevice))
	}
}
...
func handleUpgrade(_ C.uintptr_t, chatID C.int64_t, state C.ntg_media_state_struct, ptr unsafe.Pointer) {
	self := (*Client)(ptr)
	goState := MediaState{Muted: bool(state.muted), VideoPaused: bool(state.videoPaused), VideoStopped: bool(state.videoStopped), PresentationPaused: bool(state.presentationPaused)}
	for _, x0 := range self.upgradeCallbacks {
		go x0(int64(chatID), goState)
	}
}
...
func handleConnectionChange(_ C.uintptr_t, chatID C.int64_t, networkInfo C.ntg_network_info_struct, ptr unsafe.Pointer) {
	...
	for _, x0 := range self.connectionChangeCallbacks {
		go x0(int64(chatID), goCallState)
	}
}
...
func handleFrames(_ C.uintptr_t, chatID C.int64_t, streamMode C.ntg_stream_mode_enum, streamDevice C.ntg_stream_device_enum, frames *C.ntg_frame_struct, size C.uint64_t, ptr unsafe.Pointer) {
	...
	for i := uint64(0); i < uint64(size); i++ {
		...
		rawFrames[i] = Frame{Ssrc: uint32(rawFrame.ssrc), Data: C.GoBytes(unsafe.Pointer(rawFrame.data), rawFrame.sizeData), FrameData: FrameData{AbsoluteCaptureTimestampMs: int64(frames.frameData.absoluteCaptureTimestampMs), Width: uint16(frames.frameData.width), Height: uint16(frames.frameData.height), Rotation: uint16(frames.frameData.rotation)}}
	}
	for _, x0 := range self.frameCallbacks {
		go x0(int64(chatID), goStreamMode, parseStreamDevice(streamDevice), rawFrames)
	}
}
...
func handleRemoteSourceChange(_ C.uintptr_t, chatID C.int64_t, remoteSource C.ntg_remote_source_struct, ptr unsafe.Pointer) {
	self := (*Client)(ptr)
	goRemoteSource := RemoteSource{Ssrc: uint32(remoteSource.ssrc), State: parseStreamStatus(remoteSource.state), Device: parseStreamDevice(remoteSource.device)}
	for _, x0 := range self.remoteSourceCallbacks {
		go x0(int64(chatID), goRemoteSource)
	}
}
...
func handleRequestBroadcastPart(_ C.uintptr_t, chatID C.int64_t, segmentPartRequest C.ntg_segment_part_request_struct, ptr unsafe.Pointer) {
	...
	goSegmentPartRequest := SegmentPartRequest{SegmentID: int64(segmentPartRequest.segmentId), PartID: int32(segmentPartRequest.partId), Limit: int32(segmentPartRequest.limit), Timestamp: int64(segmentPartRequest.timestamp), QualityUpdate: bool(segmentPartRequest.qualityUpdate), ChannelID: int32(segmentPartRequest.channelId), Quality: goSegmentQuality}
	for _, x0 := range self.broadcastPartCallbacks {
		go x0(int64(chatID), goSegmentPartRequest)
	}
}
...
func (ctx *Client) GetState(chatId int64) (MediaState, error) {
	...
	return MediaState{Muted: bool(buffer.muted), VideoPaused: bool(buffer.videoPaused), VideoStopped: bool(buffer.videoStopped), PresentationPaused: bool(buffer.presentationPaused)}, nil
}
...
func GetProtocol() Protocol {
	...
	return Protocol{MinLayer: int32(buffer.minLayer), MaxLayer: int32(buffer.maxLayer), UdpReflector: bool(buffer.udpReflector), Versions: parseStringVector(unsafe.Pointer(buffer.libraryVersions), buffer.libraryVersionsSize)}
}
...
func GetMediaDevices() MediaDevices {
	...
	return MediaDevices{Microphone: parseDeviceInfoVector(unsafe.Pointer(buffer.microphone), buffer.sizeMicrophone), Speaker: parseDeviceInfoVector(unsafe.Pointer(buffer.speaker), buffer.sizeSpeaker), Camera: parseDeviceInfoVector(unsafe.Pointer(buffer.camera), buffer.sizeCamera), Screen: parseDeviceInfoVector(unsafe.Pointer(buffer.screen), buffer.sizeScreen)}
}
...
func (ctx *Client) Calls() map[int64]*CallInfo {
	...
	for i := 0; i < int(size); i++ {
		...
		mapReturn[int64(rawCall.chatId)] = &CallInfo{Playback: parseStreamStatus(rawCall.playback), Capture: parseStreamStatus(rawCall.capture)}
	}
	defer C.free(unsafe.Pointer(buffer))
	return mapReturn
}
